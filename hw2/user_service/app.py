...
# Create tables at startup (or use Alembic migrations in real projects)
...
def get_current_user(token: str, db: Session):
    """
    Decodes the JWT token, retrieves the 'user_id', and loads user from DB.
...
def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    """
    Register a new user given login, password, and email.
    """
...
def get_profile(authorization: str = Header(None), db: Session = Depends(get_db)):
    """
    Retrieve user profile based on the Bearer token in the 'Authorization' header.
    """
...
def update_profile(
    ...
    """
    Update user's first_name, last_name, birth_date, phone, etc.
